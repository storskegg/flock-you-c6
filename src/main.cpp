...
static bool checkRavenUUID(BLEAdvertisedDevice* device, char* out_uuid = nullptr) {
    if (!device || !device->haveServiceUUID()) return false;
    int count = device->getServiceUUIDCount();
    if (count == 0) return false;
    for (int i = 0; i < count; i++) {
        BLEUUID svc = device->getServiceUUID(i);
...
static const char* estimateRavenFW(BLEAdvertisedDevice* device) {
...
class FYBLECallbacks : public BLEAdvertisedDeviceCallbacks {
